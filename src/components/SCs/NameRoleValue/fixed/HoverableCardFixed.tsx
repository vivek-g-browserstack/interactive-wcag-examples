export function HoverableCardFixed({ interactionMethod }: { interactionMethod: "mouse" | "keyboard" }) {
    ...
    return (
        <button
            className={`text-left w-full min-h-64 h-full p-8 bg-black rounded-lg text-white outline-offset-4 group flex flex-col justify-center overflow-clip cursor-default bg-cover bg-no-repeat ${interactionMethod === "keyboard" ? `pointer-events-none` : ``}`}
